...
/**
 *
 * @description: 使用Spark-Kafka-0-10版本整合,并手动提交偏移量,维护到MySQL中
 * @author: wanjintao
 * @time: 2020/6/23 22:03
 *
 */
...
object direct_offset_mysql {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val conf = new SparkConf().setAppName("direct_offset_mysql").setMaster("local[*]")
    val sc = new SparkContext(conf)
    val ssc = new StreamingContext(sc,Seconds(sec.toInt))//每多少秒对数据进行切分形成一个RDD
    ...
    //val topics = Array("wjt9999")
    //2.使用KafkaUtil连接Kafak获取数据
    //注意:
    //如果MySQL中没有记录offset,则直接连接,从latest开始消费
    //如果MySQL中有记录offset,则应该从该offset处开始消费
    val offsetMap: mutable.Map[TopicPartition, Long] = OffsetUtil.getOffsetMap(group, topic)
  ...
  object OffsetUtil {
    ...
    def getOffsetMap(groupid: String, topic: String) = {
      val connection = DriverManager.getConnection("jdbc:mysql://10.103.104.181:3306/bigdata?characterEncoding=UTF-8&useSSL=false", "root", "Wan:1998")
